...
bool NewsDAO::prepareStatements() {
  ...
  ok &= db_.prepareStatement("delete_news", "DELETE FROM news WHERE id=$1;");

  ok &= db_.prepareStatement("select_news_by_id",
  ...
  ok &= db_.prepareStatement("select_latest_news",
                             "SELECT id, uid, title, content, source, source_logo_id, source_url, story_path, "
                             "published, link, created_at, updated_at "
                             "FROM news ORDER BY created_at DESC LIMIT 1;");

  ok &= db_.prepareStatement("select_all_news",
                             "SELECT id, uid, title, content, source, source_logo_id, source_url, story_path, "
                             "published, link, created_at, updated_at "
...
std::optional<News> NewsDAO::getById(int id) {
  std::string idStr     = std::to_string(id);
  const char* params[1] = {idStr.c_str()};
  PGresult*   res       = db_.execPrepared("select_news_by_id", 1, params);
  if (!res || PQntuples(res) == 0) {
    if (res)
      PQclear(res);
    return std::nullopt;
  }
  News news = parseNewsFromResult(res, 0);
  PQclear(res);
  return news;
}

std::optional<News> NewsDAO::getLatestNews() {
  PGresult* res = db_.execPrepared("select_latest_news", 0, nullptr);
  if (!res || PQntuples(res) == 0) {
    if (res)
      PQclear(res);
    return std::nullopt;
...
std::vector<News> NewsDAO::getAllNews() {
  std::vector<News> list;
  PGresult*         res = db_.execPrepared("select_all_news", 0, nullptr);
  if (!res)
    return list;

  int rows = PQntuples(res);
  for (int i = 0; i < rows; ++i) {
    list.push_back(parseNewsFromResult(res, i));
  }
  PQclear(res);
  return list;
}
...
bool NewsDAO::insert(const News& news) {
  ...
  PGresult* res = db_.execPrepared("insert_news", 9, params);
  if (!res)
    return false;
...
bool NewsDAO::update(const News& news) {
  ...
  PGresult* res = db_.execPrepared("update_news", 10, params);
  if (!res)
    return false;
...
bool NewsDAO::remove(int id) {
  ...
  PGresult* res = db_.execPrepared("delete_news", 1, params);
  if (!res)
    return false;
...
News NewsDAO::parseNewsFromResult(PGresult* res, int row) {
  News news;
