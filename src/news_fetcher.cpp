...
size_t NewsFetcher::write_callback(void* contents, size_t size, size_t nmemb, void* userp) {
    // Функция вызывающаяся каждый раз, когда CURL получает данные
    ((std::string*)userp)->append((char*)contents, size * nmemb);
    return size * nmemb;
}

nlohmann::json NewsFetcher::fetch_news(const std::string& url) {
    CURL* curl;
    CURLcode res;
    ...
    if (curl) {
        ...
        // Проверка на ошибки
        if (res != CURLE_OK) {
            fmt::print("[NEWS_FETCHER]: cURL error: {}\n", curl_easy_strerror(res));
        }
    ...
    if (!readBuffer.empty()) {
        try {
            ...
            fmt::print("[NEWS_FETCHER]: JSON parsing error: {}\n", e.what());
        }
    }
